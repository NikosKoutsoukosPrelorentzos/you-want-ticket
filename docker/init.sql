...
CREATE INDEX IF NOT EXISTS ix_space_id ON "space" (id);
CREATE INDEX IF NOT EXISTS ix_space_uuid ON "space" (uuid);
CREATE INDEX IF NOT EXISTS ix_space_name ON "space" (name);
...
(
    ...
    name             VARCHAR(255),
    description      VARCHAR(255),
    space_uuid       UUID REFERENCES "space" (uuid),
    created_date     TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_date     TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    capacity         INTEGER,
...
CREATE INDEX IF NOT EXISTS ix_area_id ON "area" (id);
CREATE INDEX IF NOT EXISTS ix_area_name ON "area" (name);
...
(
    ...
    owner_uuid   UUID REFERENCES "user" (uuid),
    area_uuid    UUID REFERENCES "area" (uuid),
    price        FLOAT
);
